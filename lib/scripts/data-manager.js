    ...
    if (!roots || roots.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>Ingen rødder tilføjet endnu</h3>
                <p>Klik på "Tilføj ny rod" for at komme i gang</p>
            </div>
        `;
        return;
    }
        ...
        rootElement.innerHTML = `
            ...
            <div class="accordion-content" id="root_${rootIndex}">
                <div class="areas-list">
                    ...
                    <button onclick="addNewArea(${rootIndex})" class="add-area-button">Tilføj nyt område</button>
                </div>
            </div>
        `;
...
async function renderBuildingImages() {
    ...
    const areaNameSafe = FileManager.sanitizeFilename(roots[currentRootIndex].areas[currentAreaIndex].areaName);
    let validImages = [];
...
function initializeMapPositionEditor() {
    ...
    function updatePosition(e) {
        ...
        if (currentRootIndex !== null && currentAreaIndex !== null && currentBuildingIndex !== null) {
            const building = roots[currentRootIndex].areas[currentAreaIndex].elementer[currentBuildingIndex];
            building.x = `${roundedX}%`;
            building.y = `${roundedY}%`;
        }

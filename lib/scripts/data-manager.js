    ...
    extraInfo.forEach((info, index) => {
        ...
        fieldDiv.innerHTML = `
            <div class="form-group">
                <textarea placeholder="Tilføj ekstra information...">${info || ''}</textarea>
                <button onclick="removeExtraInfoField(this)" class="remove-field">×</button>
            </div>
        `;
        container.appendChild(fieldDiv);
...
function getExtraInfoValues() {
    ...
    return Array.from(extraInfoFields)
        .map(field => field.querySelector('textarea').value.trim())
        .filter(info => info); // Only return non-empty values
}

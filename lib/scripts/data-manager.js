        ...
        <div class="area-card">
            ...
            <div class="accordion-content" id="area_${rootIndex}_${areaIndex}">
                <div class="buildings-grid">
                    ${renderBuildingsGrid(area.elementer, rootIndex, areaIndex, area.showBuildingNumbers)}
                </div>
            </div>
        </div>
    `;
...
function deleteRoot(rootIndex) {
    if (confirm('Er du sikker på, at du vil slette dette område og alle dets sider (inklusiv alle bygninger)?')) {
...
function renderBuildingsGrid(elementer, rootIndex, areaIndex, showBuildingNumbers = true) {
    return `
        ${elementer.map((building, buildingIndex) => `
            <div class="building-card" draggable="true">
                <div class="building-header">
                    ${showBuildingNumbers !== false ? `<span>${buildingIndex+1}</span>` : ''}
                    <h3>${building.navn}</h3>
...
function updateAreaDisplay(rootIndex, areaIndex) {
    ...
    if (buildingsGrid) {
        buildingsGrid.innerHTML = renderBuildingsGrid(area.elementer, rootIndex, areaIndex, area.showBuildingNumbers);
    }
...
function deleteArea(rootIndex, areaIndex) {
    if (confirm('Er du sikker på, at du vil slette denne side (inklusiv alle bygninger)?')) {
...
function deleteBuilding(rootIndex, areaIndex, buildingIndex) {
    if (confirm('Er du sikker på, at du vil slette denne bygning?')) {
...
function openBuildingModal(rootIndex, areaIndex, buildingIndex) {
    ...
    document.getElementById('buildingParking').value = building.faktaboks.parkering;  // Add this line
...
function saveBuildingChanges() {
    ...
    building.faktaboks = {
        ...
        parkering: document.getElementById('buildingParking').value  // Add this line
    };
...
function openAreaModal(rootIndex, areaIndex) {
    ...
    // Add this line to set the select value
    document.getElementById('showBuildingNumbers').value = area.showBuildingNumbers === false ? 'nej' : 'ja';
...
function saveAreaChanges() {
    ...
    // Add this line to save the showBuildingNumbers value
    area.showBuildingNumbers = document.getElementById('showBuildingNumbers').value === 'ja';
...
function addExtraInfoField() {
    ...
    fieldDiv.innerHTML = `
        <div class="form-group ekstra-info-XD">
            <textarea placeholder="Tilføj ekstra information..."></textarea>
...
function populateExtraInfo(extraInfo = []) {
    ...
    extraInfo.forEach((info, index) => {
        ...
        fieldDiv.innerHTML = `
            <div class="form-group ekstra-info-XD">
                <input type="text" 
                       placeholder="Tilføj ekstra information..." 
                       value="${info || ''}"
                />
                <button onclick="removeExtraInfoField(this)" class="remove-field">×</button>
            </div>
...
function addExtraInfoField() {
    const container = document.getElementById('extraInfoList');
    const fieldDiv = document.createElement('div');
    fieldDiv.className = 'ekstra-info-XD'; // Changed from 'extra-info-field'
    fieldDiv.innerHTML = `
        <input type="text" placeholder="Tilføj ekstra information..."/>
        <button onclick="removeExtraInfoField(this)" class="remove-field">×</button>
    `;
    container.appendChild(fieldDiv);
}
...
function removeExtraInfoField(button) {
    const field = button.closest('.ekstra-info-XD'); // Changed from 'extra-info-field'
    field.remove();
}

function getExtraInfoValues() {
    const extraInfoFields = document.querySelectorAll('.ekstra-info-XD'); // Changed selector
    return Array.from(extraInfoFields)
